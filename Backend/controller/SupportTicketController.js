...
export const createTicket = async (req, res) => {
  try {
    ...
    const newTicket = new SupportTicket({
        ...
        message,
        referenceCode,
        createdBy: req.user.email, // Track who created the ticket
    });
...
export const getAllTickets = async (req, res) => {
  try {
    ...
    // Only allow admin/staff to see all tickets
    if (!req.user.role || !['admin', 'staff'].includes(req.user.role.toLowerCase())) {
      return res.status(403).json({ message: 'Access denied. Admin/Staff privileges required.' });
    }
