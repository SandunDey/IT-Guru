  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      const addField = (label, value) => {
        ...
        pdf.setFont('helvetica', 'normal');
        const fieldValue = value || 'Not provided';
        pdf.text(fieldValue, margin + 40, yPosition);
        yPosition += 8;
      };
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6">
          ...
          <div className="flex items-center space-x-2">
            <button 
              ...
              className="p-2 bg-white rounded-md shadow-sm hover:bg-gray-200 disabled:bg-gray-300 disabled:cursor-not-allowed transition-colors"
              title="Download PDF"
            >
              {isDownloading ? (
            ...
            {!['Resolved', 'Closed'].includes(ticket.status) && (
              <button 
                ...
                className="py-2 px-4 text-sm font-medium text-white bg-red-600 rounded-md hover:bg-red-700 transition-colors"
              >
                Close Ticket
              </button>
            )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 bg-white p-6 rounded-lg shadow-md">
            <div className="border-b pb-4 mb-4">
              ...
              <p className="mt-4 text-gray-700 whitespace-pre-wrap leading-relaxed">{ticket.message}</p>
            </div>

            {/* Staff Replies Section */}
            {ticket.replies && ticket.replies.length > 0 ? (
              <div className="mt-6">
                <h3 className="font-semibold text-gray-800 mb-3 text-lg">Staff Replies</h3>
                {ticket.replies.map((reply, index) => (
                  <div key={index} className="bg-blue-50 p-4 rounded-lg mb-3 border border-blue-100">
                    <div className="flex justify-between items-start mb-2">
                    ...
                    <p className="text-blue-700 whitespace-pre-wrap leading-relaxed">{reply.text}</p>
                  </div>
                ))}
              </div>
            ) : (
              <div className="mt-6">
                <p className="text-center text-gray-400 font-semibold p-4 border-2 border-dashed rounded-md bg-gray-50">
                  No staff replies yet. Replies will appear here when available.
                </p>
              </div>
            )}
          </div>
          ...
          <div className="space-y-6">
            <div className="bg-white p-6 rounded-lg shadow-md">
              <h3 className="font-bold text-lg mb-4 border-b pb-2 text-gray-800">Ticket Information</h3>
              <div className="space-y-3 text-sm">
                <div className="flex justify-between items-center py-1 border-b border-gray-100">
                  <span className="font-semibold text-gray-600">Reference:</span>
                  <span className="font-mono text-gray-800 bg-gray-100 px-2 py-1 rounded">{ticket.referenceCode}</span>
                </div>
                <div className="flex justify-between items-center py-1 border-b border-gray-100">
                  <span className="font-semibold text-gray-600">Reg Number:</span>
                  <span className="text-gray-800">{ticket.registrationNumber}</span>
                </div>
                <div className="flex justify-between items-center py-1 border-b border-gray-100">
                  <span className="font-semibold text-gray-600">Contact:</span>
                  <span className="text-gray-800">{ticket.contactNumber || 'Not provided'}</span>
                </div>
                <div className="flex justify-between items-center py-1 border-b border-gray-100">
                  <span className="font-semibold text-gray-600">Course/Year:</span>
                  <span className="text-gray-800">{ticket.courseOrExamYear}</span>
                </div>
                <div className="flex justify-between items-center py-1">
                  <span className="font-semibold text-gray-600">Department:</span>
                  <span className="text-gray-800">Student Services</span>
                </div>
              </div>
            </div>
          </div>
      ...
      {showConfirmModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
          <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-md mx-4">
            <h2 className="text-xl font-bold mb-4 text-gray-800">Close Ticket</h2>
            <p className="text-gray-600 mb-6">Are you sure you want to close this ticket? This action cannot be undone.</p>
